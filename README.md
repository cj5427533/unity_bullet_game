...
- **오브젝트 풀링 도입**: 총알/아이템의 Instantiate/Destroy 비용 및 GC 스파이크 완화
- **기록 영구 저장**: `PlayerPrefs` 또는 JSON 파일로 난이도별 최고 기록 저장
- **난이도 밸런싱**: 페이즈 증가 곡선(속도/발사 간격/패턴)을 데이터 기반으로 조정 가능하게 개선
- **입력/조작감 개선**: 카메라/가속/감속/대시 등 플레이 감각 튜닝
- **UI/UX 개선**: 일시정지, 재시작, 결과 화면(리트라이 동선) 정리
